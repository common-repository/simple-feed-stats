<?php // Simple Statistics for Feeds - Admin Functions
...
function sfs_add_options_page() {
	...
	add_options_page('Simple Statistics for Feeds', 'Feed Statistics', 'manage_options', 'sfs-options', 'sfs_render_form');
...
function sfs_plugin_action_links($links, $file) {
	...
	if ($file === plugin_basename(SFS_PLUGIN_FILE) && current_user_can('manage_options')) {
...
function add_sfs_links($links, $file) {
	...
	if ($file === plugin_basename(SFS_PLUGIN_FILE)) {
		...
		$home_href  = 'https://perishablepress.com/simple-feed-stats/';
		$home_title = esc_attr__('Plugin Homepage', 'simple-feed-stats');
		$home_text  = esc_html__('Homepage', 'simple-feed-stats');
		
		$links[] = '<a target="_blank" rel="noopener noreferrer" href="'. $home_href .'" title="'. $home_title .'">'. $home_text .'</a>';
		
		$rate_href  = 'https://wordpress.org/support/plugin/simple-feed-stats/reviews/?rate=5#new-post';
		$rate_title = esc_html__('Give us a 5-star rating at WordPress.org', 'simple-feed-stats');
		$rate_text  = esc_html__('Rate this plugin', 'simple-feed-stats') .'&nbsp;&raquo;';
...
// custom footer text
function sfs_admin_footer_text($text) {
	
	if (!function_exists('get_current_screen')) require_once ABSPATH .'/wp-admin/includes/screen.php';
	
	$screen_id = sfs_get_current_screen_id();
	
	$ids = array('settings_page_sfs-options');
	
	if ($screen_id && apply_filters('sfs_admin_footer_text', in_array($screen_id, $ids))) {
		
		$text = __('Like this plugin? Give it a', 'simple-feed-stats');
		
		$text .= ' <a target="_blank" rel="noopener noreferrer" href="https://wordpress.org/support/plugin/simple-feed-stats/reviews/?rate=5#new-post">';
		
		$text .= __('★★★★★ rating&nbsp;&raquo;', 'simple-feed-stats') .'</a>';
...
add_filter('admin_footer_text', 'sfs_admin_footer_text', 10, 1);	



// get_current_screen proper
function sfs_get_current_screen_id() {
...
function sfs_version_control() {
	...
	if ($screen_id !== 'settings_page_sfs-options') return;
...
function sfs_dashboard_widget() {
	...
	<style type="text/css">
		.sfs-table table,
		.sfs-table th { border: 0; border-collapse: collapse; }
		.sfs-table th { padding: 5px 15px; font-size: 12px; text-transform: uppercase; }
		.sfs-table td { 
			display: table-cell; vertical-align: middle; padding: 10px 15px; color: #555; border: 1px solid #dfdfdf;
	...
	<div class="sfs-table">
		<table class="widefat">
			...
			<tbody>
				<tr>
					<td class="rdf"><?php      echo number_format($sfs_query_current[0]); ?></td>
					<td class="rss2"><?php     echo number_format($sfs_query_current[1]); ?></td>
					<td class="atom"><?php     echo number_format($sfs_query_current[2]); ?></td>
					<td class="comments"><?php echo number_format($sfs_query_current[3]); ?></td>
				</tr>
			</tbody>
		</table>
	</div>
...
function add_custom_dashboard_widget() {
	...
	wp_add_dashboard_widget('sfs_dashboard_widget', 'Feed Statistics', 'sfs_dashboard_widget');
...
function sfs_render_form() {
	...
	<style type="text/css">
		...
		.sfs-admin abbr[title] { cursor: help !important; border-bottom: 1px dotted #999 !important; text-decoration: none !important; line-height: 1; }
		...
		.dismiss-alert .description { display: inline-block; margin: 0 15px 0 0; }
		.dismiss-alert .input-alert[type=checkbox] { margin: -2px 4px 0 0; }
		
		.toggle { padding: 0 15px 15px 15px; }
		.toggle.sfs-overview {
			padding: 0 15px 20px 140px;
			background-image: url(<?php echo plugins_url('/simple-feed-stats/sfs-logo.jpg'); ?>); 
			background-repeat: no-repeat; background-position: 10px 0; background-size: 120px 131px;
			}
		.toggle.sfs-overview p { margin: 0; }
		.toggle.toggle-support { padding: 0 15px 0 0; }
		.toggle.toggle-support p { margin-left: 15px; }
		
		.sfs-admin .sfs-menu-item { float: left; margin: 12px 36px 12px 0; }
		.sfs-admin .sfs-sub-item { display: inline-block; vertical-align: middle; }
		.sfs-admin .sfs-sub-item select, .sfs-admin .sfs-sub-item input { vertical-align: middle; }
		.sfs-admin .sfs-menu-row { margin: 12px 0 0 0; }
		...
		.sfs-admin .sfs-table table,
		.sfs-admin .sfs-table th { border: 0; border-collapse: collapse; }
		.sfs-admin .sfs-table th { padding: 5px 15px; font-size: 12px; text-transform: uppercase; }
		.sfs-admin .sfs-table td { padding: 10px; color: #555; border: 1px solid #dfdfdf; }
		.sfs-admin .sfs-table td.sfs-type { display: table-cell; vertical-align: middle; padding: 15px; text-align: left; text-shadow: 1px 1px 1px #fff; font: bold 20px/20px Georgia, serif; }
		
		.sfs-admin .sfs-statistics div { margin: 5px; }
		.sfs-admin .sfs-stats-type { font-weight: bold; }
		.sfs-admin .total-pages { display: inline-block; margin: 0 3px 0 2px; vertical-align: middle; position: relative; top: 1px; }
		
		.sfs-admin .rdf      { background-color: #d9e8f9; }
		.sfs-admin .rss2     { background-color: #d5f2d5; }
		.sfs-admin .atom     { background-color: #fafac0; }
		.sfs-admin .comments { background-color: #fee6cc; }
		.sfs-admin .open     { background-color: #ffe3e3; }
		.sfs-admin .other    { background-color: #efefef; }
		
		/**/
		
		.sfs-table .form-table td { border: 0; }
		.sfs-table .form-table th { display: table-cell; vertical-align: middle; padding: 10px; font-size: 14px; text-transform: none; }
		.sfs-table-item { margin: 0 0 5px 0; }
		.sfs-last-item { margin: 24px 0 0 0; }
		.label-desc { margin: 5px 0; }
		
		.sfs-admin .sfs-open-tracking-url, 
		.sfs-admin .sfs-open-tracking-image { background-color: #f7f7f7; }
		
		.sfs-admin .sfs-open-tracking-url td   { padding-top: 24px; }
		.sfs-admin .sfs-open-tracking-image td { padding-bottom: 24px; }
		
		.sfs-admin .sfs-open-tracking-url   input[type=text], 
		.sfs-admin .sfs-open-tracking-image input[type=text], 
		.sfs-admin textarea.code { width: 80%; min-width: 250px; max-width: 800px; font-size: 12px; color: #777; }
		.sfs-admin textarea.code { height: 80px; min-height: 50px; }
		
		.sfs-radio { margin: 5px 0; }
		.sfs-radio * { vertical-align: middle; }
		.sfs-radio input[type=radio] { margin-bottom: -2px; }
		.sfs-admin input[type=checkbox] { margin-top: -3px; }
		.sfs-admin input[type=text] { width: 180px; }
		
		.sfs-admin .mm-ua .mm-code { display: inline; }
		.sfs-admin .mm-code {
			display: inline-block; margin: 0; padding: 1px 3px; direction: ltr; unicode-bidi: embed;
			color: #333; background-color: #eaeaea; background-color: rgba(0,0,0,0.05);
			line-height: 22px; font-size: 12px; font-family: Consolas, Monaco, monospace;
			}
		
		.tooltip { 
			cursor: help; display: inline-block; width: 18px; height: 18px; margin: 0 0 0 2px; text-align: center; font: bold 12px/18px Georgia, serif;
			border: 2px solid #fff; color: #fff; background-color: #b0c6d0; border-radius: 18px;
		...
		#easyTooltip { 
			...
			#easyTooltip code { padding: 2px; line-height: 0; font-size: 90%; }
		
		.sfs-admin .sfs-credits { margin: -10px 0 10px 5px; font-size: 12px; }
		
		<?php echo sfs_default_badge_styles(); ?>
		...
		<div class="metabox-holder">
			<div class="meta-box-sortables ui-sortable">
				...
				<div <?php echo $display_alert; ?>class="postbox">
					<h2><?php esc_html_e('This plugin needs your support!', 'simple-feed-stats'); ?></h2>
					<div class="toggle">
				...
				<div class="postbox">
					...
					<div class="toggle sfs-overview<?php if (isset($_GET['settings-updated']) || isset($_GET['cache']) || isset($_GET['filter']) || isset($_GET['jump']) || isset($_GET['reset']) || isset($_GET['p'])) echo ' default-hidden'; ?>">
						<p>
							<?php esc_html_e('Simple Statistics for Feeds tracks your feeds, adds custom content, and displays your feed statistics on your site.', 'simple-feed-stats'); ?> 
							<?php esc_html_e('SFS tracks your feeds automatically and displays the statistics on this page and via the Dashboard widget. Note: this plugin was formerly named &ldquo;Simple Feed Stats&rdquo;.', 'simple-feed-stats'); ?> 
						</p>
						<ul>
					...
					<div class="toggle default-hidden">
						...
						<div class="sfs-table">
							<table class="widefat">
								...
								<tbody>
									<tr>
										<td class="sfs-type rdf"><?php      echo esc_attr(number_format($sfs_query_current[0])); ?></td>
										<td class="sfs-type rss2"><?php     echo esc_attr(number_format($sfs_query_current[1])); ?></td>
										<td class="sfs-type atom"><?php     echo esc_attr(number_format($sfs_query_current[2])); ?></td>
										<td class="sfs-type comments"><?php echo esc_attr(number_format($sfs_query_current[3])); ?></td>
										<td class="sfs-type open"><?php     echo esc_attr(number_format($sfs_query_current[4])); ?></td>
										<td class="sfs-type other"><?php    echo esc_attr(number_format($sfs_query_current[5])); ?></td>
									</tr>
								</tbody>
							</table>
						</div>
					...
					<div class="toggle default-hidden">
						...
						<div class="sfs-table">
							<table class="widefat">
								...
								<tbody>
									<tr>
										<td class="sfs-type rdf"><?php      echo esc_attr(number_format($sfs_query_alltime[0])); ?></td>
										<td class="sfs-type rss2"><?php     echo esc_attr(number_format($sfs_query_alltime[1])); ?></td>
										<td class="sfs-type atom"><?php     echo esc_attr(number_format($sfs_query_alltime[2])); ?></td>
										<td class="sfs-type comments"><?php echo esc_attr(number_format($sfs_query_alltime[3])); ?></td>
										<td class="sfs-type open"><?php     echo esc_attr(number_format($sfs_query_alltime[4])); ?></td>
										<td class="sfs-type other"><?php    echo esc_attr(number_format($sfs_query_alltime[5])); ?></td>
									</tr>
								</tbody>
							</table>
						</div>
				...
				<div class="postbox">
					...
					<div class="toggle<?php if (!isset($_GET['filter']) && !isset($_GET['jump']) && !isset($_GET['p'])) echo ' default-hidden'; ?>">
						...
						<div class="sfs-menu-item">
							...
							<form class="sfs-sub-item" action="">
								<input type="hidden" name="page" value="sfs-options">
								<input class="current-page small-text" name="jump" type="number" size="3" value="<?php echo esc_attr($pagevar); ?>"> 
							...
							<div class="sfs-table">
								<table class="form-table">
									...
									<tr class="sfs-open-tracking-url<?php if ($sfs_options['sfs_tracking_method'] !== 'sfs_open_tracking') echo ' default-hidden'; ?>">
										...
										<td>
											<div class="sfs-table-item">
												<?php esc_html_e('For Open Tracking. Use this URL as the', 'simple-feed-stats'); ?> 
												<span class="mm-code">src</span> <?php esc_html_e('for your tracking image:', 'simple-feed-stats'); ?> 
									...
									<tr class="sfs-open-tracking-image<?php if ($sfs_options['sfs_tracking_method'] !== 'sfs_open_tracking') echo ' default-hidden'; ?>">
										...
										<td>
											<div class="sfs-table-item">
												<?php esc_html_e('For Open Tracking. Optional custom tracking image:', 'simple-feed-stats'); ?> 
							...
							<div class="sfs-table">
								<table class="form-table">
									<tr>
										...
										<td>
											<input class="small-text" type="number" min="1" max="999" name="sfs_options[sfs_number_results]" value="<?php echo esc_attr($sfs_options['sfs_number_results']); ?>" /> 
							...
							<div class="sfs-table">
								<table class="form-table">
									<tr>
										...
										<td>
											<textarea class="large-text code" cols="50" rows="3" name="sfs_options[sfs_custom_styles]"><?php echo esc_textarea($sfs_options['sfs_custom_styles']); ?></textarea>
									...
									<tr>
										...
										<td>
											<textarea class="large-text code" cols="50" rows="3" name="sfs_options[sfs_feed_content_before]"><?php echo esc_textarea($sfs_options['sfs_feed_content_before']); ?></textarea>
											<div class="label-desc">
									...
									<tr>
										...
										<td>
											<textarea class="large-text code" cols="50" rows="3" name="sfs_options[sfs_feed_content_after]"><?php echo esc_textarea($sfs_options['sfs_feed_content_after']); ?></textarea>
											<div class="label-desc">
												<?php esc_html_e('Text and basic markup allowed', 'simple-feed-stats'); ?> 
												<span class="tooltip" title="<?php esc_attr_e('Tip: you can has shortcodes. Leave blank to disable.', 'simple-feed-stats'); ?>">?</span>
											</div>
				...
				<div class="postbox">
					...
					<div class="toggle default-hidden">
						...
						<div class="sfs-table">
							<ul>
								<li><?php esc_html_e('Content RDF',   'simple-feed-stats'); ?> &ndash; <a target="_blank" rel="noopener noreferrer" href="<?php echo esc_attr($feed_rdf);       ?>"><span class="mm-code"><?php echo esc_url($feed_rdf);       ?></span></a></li>
								<li><?php esc_html_e('Content RSS2',  'simple-feed-stats'); ?> &ndash; <a target="_blank" rel="noopener noreferrer" href="<?php echo esc_attr($feed_rss2);      ?>"><span class="mm-code"><?php echo esc_url($feed_rss2);      ?></span></a></li>
								<li><?php esc_html_e('Content Atom',  'simple-feed-stats'); ?> &ndash; <a target="_blank" rel="noopener noreferrer" href="<?php echo esc_attr($feed_atom);      ?>"><span class="mm-code"><?php echo esc_url($feed_atom);      ?></span></a></li>
								<li><?php esc_html_e('Comments RSS2', 'simple-feed-stats'); ?> &ndash; <a target="_blank" rel="noopener noreferrer" href="<?php echo esc_attr($feed_coms);      ?>"><span class="mm-code"><?php echo esc_url($feed_coms);      ?></span></a></li>
								<li><?php esc_html_e('Comments Atom', 'simple-feed-stats'); ?> &ndash; <a target="_blank" rel="noopener noreferrer" href="<?php echo esc_attr($feed_coms_atom); ?>"><span class="mm-code"><?php echo esc_url($feed_coms_atom); ?></span></a></li>
							</ul>
						</div>
						<p><strong><?php esc_html_e('More about WordPress feeds', 'simple-feed-stats'); ?></strong></p>
						<ul>
							<li><a target="_blank" rel="noopener noreferrer" href="https://wordpress.org/plugins/simple-feed-stats/"><?php esc_html_e('Simple Statistics for Feeds Homepage', 'simple-feed-stats'); ?></a></li>
							<li><a target="_blank" rel="noopener noreferrer" href="https://wordpress.org/support/article/wordpress-feeds/"><?php esc_html_e('WP Codex: WordPress Feeds', 'simple-feed-stats'); ?></a></li>
							...
							<li><a target="_blank" rel="noopener noreferrer" href="https://feedburner.google.com/"><?php esc_html_e('Google/Feedburner', 'simple-feed-stats'); ?></a></li>
						</ul>
						<p><strong><?php esc_html_e('Your browser/IP info', 'simple-feed-stats'); ?></strong></p>
						<ul>
							<li><?php esc_html_e('IP Address:', 'simple-feed-stats'); ?> <span class="mm-code"><?php echo sfs_clean($address); ?></span></li>
							<li>
								<?php esc_html_e('Approx. Time:', 'simple-feed-stats'); ?> <span class="mm-code"><?php echo sfs_clean($curtime); ?></span>
							...
							<li class="mm-ua"><?php esc_html_e('User Agent:', 'simple-feed-stats'); ?> <span class="mm-code"><?php echo sfs_clean($agent); ?></span></li>
						</ul>
					</div>
				</div>
				...
				<div id="sfs-shortcodes" class="postbox">
					...
					<div class="toggle default-hidden">
						...
						<p><span class="mm-code">[sfs_subscriber_count]</span></p>
						...
						<p><span class="mm-code">[sfs_count_badge]</span></p>
						...
						<p><span class="mm-code">[sfs_count_simple]</span></p>
						<p><span class="mm-code">[sfs_count_simple link="https://example.com/feed/"]</span></p>
						
						<p><?php esc_html_e('Display daily count for RSS2 feeds in plain-text:', 'simple-feed-stats'); ?></p>
						<p><span class="mm-code">[sfs_rss2_count]</span></p>
						
						<p><?php esc_html_e('Display daily count for comment feeds in plain-text:', 'simple-feed-stats'); ?></p>
						<p><span class="mm-code">[sfs_comments_count]</span></p>
						
						
						<h3><?php esc_html_e('Template Tags', 'simple-feed-stats'); ?></h3>
						
						<p><?php esc_html_e('Display daily count for all feeds in plain-text:', 'simple-feed-stats'); ?></p>
						<p><span class="mm-code">&lt;?php if (function_exists('sfs_display_subscriber_count')) sfs_display_subscriber_count(); ?&gt;</span></p>
						
						<p>
							<?php esc_html_e('Display daily count for all feeds with a FeedBurner-style badge:', 'simple-feed-stats'); ?>
							<span class="tooltip" title="<?php esc_attr_e('Tip: visit the &ldquo;Plugin Settings&rdquo; panel to style your badge with some custom CSS. Also, check out the plugin FAQs for info on wrapping the FeedBurner count badge with a link to your feed.', 'simple-feed-stats'); ?>">?</span>
						</p>
						<p><span class="mm-code">&lt;?php if (function_exists('sfs_display_count_badge')) sfs_display_count_badge(); ?&gt;</span></p>
						...
						<p><span class="mm-code">&lt;?php if (function_exists('sfs_display_count_simple')) sfs_display_count_simple(); ?&gt;</span></p>
						<p><span class="mm-code">&lt;?php if (function_exists('sfs_display_count_simple')) sfs_display_count_simple(array('link' => 'https://example.com/feed/')); ?&gt;</span></p>
						
						<p><?php esc_html_e('Display total count for all feeds as plain-text:', 'simple-feed-stats'); ?></p>
						<p><span class="mm-code">&lt;?php if (function_exists('sfs_display_total_count')) sfs_display_total_count(); ?&gt;</span></p>
				...
				<div class="postbox wp-resources">
					<h2><?php esc_html_e('WP Resources', 'simple-feed-stats'); ?></h2>
